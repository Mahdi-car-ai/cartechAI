...
export const getCommunityChat = async (chatId: string): Promise<CommunityChat | null> => {
  try {
...
export const createCommunityChat = async (topic: string, description: string): Promise<CommunityChat | null> => {
  try {
    const response = await api.post('/chats/create-community', { topic, description });
    return response.data as CommunityChat;
  } catch (error) {
