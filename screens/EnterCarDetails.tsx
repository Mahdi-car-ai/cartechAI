...
export default function EnterCarDetailsScreen() {
  ...
  const renderCarDetailField = (key: string) => {
    ...
    if (key === "engineDisplacement") {
      ...
      const displayValue = focusedInput === key ? engineDisplacementRaw : carDetails[key];
    ...
    return (
      ...
      >
        <Text style={[styles.inputText, !carDetails[key] && styles.placeholderText]}>
          {carDetails[key] || key
            .replace(/([A-Z])/g, " $1")
            .trim()
            .replace(/\b\w/g, (char) => char.toUpperCase())}
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            {loading ? (
              <ActivityIndicator size="large" color="#95ff77" style={styles.loader} />
            ) : (
              renderModalList()
            )}
          </View>
